...
export function useLeads(filters?: { status?: string; city?: string; service_type?: string; urgency?: string; search?: string }) {
  return useQuery({
    queryKey: ["leads", filters],
    queryFn: async () => {
      let query = supabase.from("leads").select("*, buyers(business_name)").order("created_at", { ascending: false });
